...
const treeToShrinkwrap = require('npm-shrinkwrap').treeToShrinkwrap
const packageId = require('./packid.js')
...
const opti = require('./opti.js')
const url = require('url')
const npa = require('npm-package-arg')
const uuid = require('uuid')
const ssri = require('ssri')
const cloneDeep = require('lodash.clonedeep')
const pacoteOpts = require('pacote')
...
function submitForInstallReport (auditData) {
  ...
  return gzip(JSON.stringify(auditData)).then(body => {
    opti.emit('timeEnd', 'tenax compress')
    log.info('tenax', 'Submitting payload of ' + body.length + 'bytes')
    ...
    opti.emit('time', 'tenax submit')
    return fetchAudit('/-/npm/v1/security/audits/quick', body).then(response => {
      opti.emit('timeEnd', 'tenax submit')
      opti.emit('time', 'tenax body')
      return response.json()
    }).then(result => {
      opti.emit('timeEnd', 'tenax body')
      return result
    })
  })
}
