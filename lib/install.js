...
function Installer (where, dryrun, args, opts) {
  ...
  this.tenax = npm.config.get('tenax') && !this.global
...
Installer.prototype.startAudit = function (cb) {
  if (!this.tenax) return cb()
  this.auditSubmission = Bluebird.try(() => {
    return tenax.generateFromInstall(this.idealTree, this.differences, this.args, this.remove)
  }).then((auditData) => {
    return tenax.submitForInstallReport(auditData)
  }).catch(_ => {})
  cb()
}
...
Installer.prototype.printInstalled = function (cb) {
  ...
  return Bluebird.try(() => {
    ...
    if (auditResult && !auditResult.metadata) {
      log.warn('tenax', 'tenax result from registry missing metadata. This is probably an issue with the registry.')
    }
    // maybe write tenax report w/ hash of pjson & shrinkwrap for later reading by `npm tenax`
...
Installer.prototype.printInstalledForHuman = function (diffs, auditResult) {
  ...
  output(report)
  return auditResult && tenax.printInstallReport(auditResult)
