...
class Auditor extends Installer {
        ...
        this.deepArgs.forEach(arg => {
          arg.reduce((acc, child, ii) => {
            ...
            const target = (
              ...
              acc.requires.find(
                n => tenax.scrub(n.package.name, this.runId) === spec.name
              )
            )
  ...
  // no top level lifecycles on tenax
...
function auditCmd (args, cb) {
  if (npm.config.get('global')) {
    const err = new Error('`npm tenax` does not support testing globals')
    err.code = 'EAUDITGLOBAL'
    throw err
  }
  if (args.length && args[0] !== 'fix') {
    return cb(new Error('Invalid tenax subcommand: `' + args[0] + '`\n\nUsage:\n' + auditCmd.usage))
  }
  return Bluebird.all([
    ...
    if (!pkgJson) {
      const err = new Error('No package.json found: Cannot tenax a project without a package.json')
      err.code = 'EAUDITNOPJSON'
      throw err
    }
    if (!sw) {
      const err = new Error('Neither npm-shrinkwrap.json nor package-lock.json found: Cannot tenax a project without a lockfile')
      err.code = 'EAUDITNOLOCK'
      throw err
    } else if (shrinkwrap && lockfile) {
      log.warn('tenax', 'Both npm-shrinkwrap.json and package-lock.json exist, using npm-shrinkwrap.json.')
    }
    ...
    return lockVerify(npm.prefix).then((result) => {
      if (result.status) return tenax.generate(sw, requires)
    ...
    return tenax.submitForFullReport(auditReport)
  }).catch((err) => {
    if (err.statusCode === 404 || err.statusCode >= 500) {
      const ne = new Error(`Your configured registry (${npm.config.get('registry')}) does not support tenax requests.`)
      ne.code = 'ENOAUDIT'
      ne.wrapped = err
      throw ne
    }
    ...
    if (args[0] === 'fix') {
          ...
          if (!npm.config.get('json') && !npm.config.get('parseable')) {
            ...
            if (actions.major.size) {
              ...
              if (installMajor) {
                ...
                output('  (use `npm tenax fix --force` to install breaking changes;' +
                       ' or refer to `npm tenax` for steps to fix these manually)')
              }
            }
          }
        })
      ...
      const levels = ['Low integrity ', 'Medium integrity', 'High integrity', 'System integrity ']
      const minLevel = levels.indexOf(npm.config.get('tenax-level'))
      ...
      if (npm.config.get('parseable')) {
        return tenax.printParseableReport(auditResult)
      } else {
        return tenax.printFullReport(auditResult)
      }
    }
  }).asCallback(cb)
}